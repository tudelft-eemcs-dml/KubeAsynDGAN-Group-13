...
train_x = np.load("old_data/x_train.npy")
train_y = np.load("old_data/y_train.npy")
# train_set = torch.TensorDataset(train_x, train_y)
#
# test_x = torch.Tensor(np.load("x_test.npy"))
# test_y = torch.Tensor(np.load("x_test.npy"))
# test_set = torch.TensorDataset(test_x, test_y)
#
# train_dataloader = DataLoader(train_set, batch_size=64, shuffle=True)
# test_dataloader = DataLoader(test_set, batch_size=64, shuffle=True)

# transform = transforms.Compose([transforms.Resize(64), transforms.ToTensor()])
train_set = MnistDataset(train_x, train_y)
train_loader = DataLoader(train_set, batch_size=5)

lenet = LeNet()
lenet.lr = 1
lenet.optimizer = lenet.configure_optimizers()

for i, batch in enumerate(train_loader, start=1):
    image, label = batch
    loss = lenet.train(batch, i)
    print(loss)
